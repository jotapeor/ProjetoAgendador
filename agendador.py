...
with tab3:
    ...
    if not st.session_state.compromisso:
        ...
        for nome, dados in st.session_state.compromisso.items():
            compromissos_para_exibir.append(
                {"Compromisso": nome, "Data": dados["data"], "Hora": dados["hora"]})
        ...
        novo_nome = st.text_input(
            "Novo nome do compromisso (deixe vazio para não alterar):")
        nova_data = st.date_input(
            "Nova data do compromisso (deixe igual para não alterar):")
        nova_hora = st.time_input(
            "Nova hora do compromisso (deixe igual para não alterar):")
        if st.button("Editar compromisso"):
            if nome_cancelar in st.session_state.compromisso:
                if novo_nome:
                    st.session_state.compromisso[novo_nome] = st.session_state.compromisso.pop(
                        nome_cancelar)
                    nome_cancelar = novo_nome
                st.session_state.compromisso[nome_cancelar]["data"] = str(
                    nova_data)
                st.session_state.compromisso[nome_cancelar]["hora"] = str(
                    nova_hora)
                salvar_dados()
